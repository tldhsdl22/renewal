...
class GmapViewController:UIViewController
{
    ...
    var dist = 500
    {
        didSet(oldVal)
        {
            ...
            {
                strDist = String(dist) + "m"
            }
            else
            {
                strDist = String(Int(dist / 1000)) + "km"
            }
            
            distBtn.setTitle(strDist, for: .normal)
        }
    }
    ...
    @IBAction func unwindToGMapViewControllerbFromGMapDistanceViewController(_ segue: UIStoryboardSegue){
        if let vc = segue.source as? GMapDistanceViewController {
            dist = vc.distance
        }
        makePicker()
    ...
    private func loadData(location:CLLocation?)
    {
        ...
        let params = ["x":longitude, "y":latitude, "distance":dist] as [String : Any]
        print(params)
        Alamofire.request("http://www.iloveggbb.com/nearStore/index.php", method: .post, parameters: params).responseJSON(completionHandler: ({ (response) in
            print(response)
    ...
    private func setLayout()
    {
        ...
        myLocationBtn.setImage(UIImage(named:"my_location_on"), for: .highlighted)
...
extension GmapViewController:UICollectionViewDataSource, UICollectionViewDelegateFlowLayout
{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let storyboard = UIStoryboard(name: "StoreInfo", bundle: nil)
        if let vc = storyboard.instantiateViewController(withIdentifier: "StoreInfoViewController") as? StoreInfoViewController
        {
            vc.storeLocation = storeList?[indexPath.row].location ?? ""
            vc.storeTitle = storeList?[indexPath.row].name ?? ""
            vc.storeType = storeList?[indexPath.row].type ?? ""
            vc.storeNum = storeList?[indexPath.row].num ?? ""
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "GMapCollectionViewCell", for: indexPath) as? GMapCollectionViewCell
        {
            cell.setLayout()
...
class GMapCollectionViewCell:UICollectionViewCell
{
